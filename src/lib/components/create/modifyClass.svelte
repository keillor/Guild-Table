<script>
    ...
    for (let key in classData) {
        if(key.endsWith('choices')) {
            options.push({data: classData[key], displayValue: key.replace(/_/g, ' ').replace(/\b\w/g, char => char.toUpperCase()), key: key});
        }
    }
</script>
        ...
        <Card.Content>
            {#each options as thing (thing.key)}
                {#each thing.data as option (option.desc)}
                    <ClassOptionSelect formInputName={option.desc} formDisplayName={option.desc} form={form} choices={option}/> 
                {/each}
            {/each}
        </Card.Content>
    </Card.Root>
